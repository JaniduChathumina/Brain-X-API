...
class PixelImportance(ExplanationBase):
    ...
    def plot(self, index=None, class_names=None, max_num_figures=20, **kwargs):
        ...
        if len(indices) > 10:
            warnings.warn(
                f"There are too many instances ({len(indices)} > 10), "
                f"so only the first 10 instances are plotted."
            )
            indices = indices[:10]
        if len(indices) == 0:
            return [], []
        ...
        for index in indices:
            ...
            for i, scores in enumerate(all_scores):
                ...
                # Image and importance scores for heatmap
                if not self.use_heatmap:
                    heatmap_scores = _plot_pixel_importance(importance_scores, image, overlay=True)
                else:
                    heatmap_scores = _plot_pixel_importance_heatmap(importance_scores, image, overlay=True)
                ax_heatmap.imshow(heatmap_scores)
                ax_heatmap.set_xticks([])
                ax_heatmap.set_yticks([])
                if "labels" in exp:
                    ax_heatmap.set_title(f"Heatmap - {labels[i]}")
                elif "target_label" in exp:
                    label = exp["target_label"]
                    class_name = label if class_names is None else class_names[label]
                    ax_heatmap.set_title(f"Heatmap - {class_name}")

                # Positive pixel importance scores for score plot
                if not self.use_heatmap:
                    score_scores = _plot_pixel_importance(importance_scores, image, polarity="positive")
                else:
                    score_scores = _plot_pixel_importance_heatmap(importance_scores, image, overlay=False)
                ax_score.imshow(score_scores)
                ax_score.set_xticks([])
                ax_score.set_yticks([])
                ax_score.set_title("Score")

                heatmap_figures.append(fig_heatmap)
                score_figures.append(fig_score)

                if len(heatmap_figures) >= max_num_figures:
                    return heatmap_figures, score_figures
